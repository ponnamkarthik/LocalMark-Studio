  ...
  useEffect(() => {
    ...
    const renderDiagram = async () => {
      try {
        ...
        // Validate syntax (throws on errors)
        try {
          await (mermaid as any).parse(definition);
        } catch (parseError) {
          throw parseError;
        }
        ...
        // Mermaid v11 supports a new object signature; keep a fallback for older signatures.
        let result: any;
        try {
          result = await (mermaid as any).render({ id, text: definition });
        } catch {
          result = await (mermaid as any).render(id, definition);
        }

        const nextSvg: string =
          typeof result === "string" ? result : result?.svg ?? "";
        const bindFunctions: undefined | ((el: Element) => void) =
          result?.bindFunctions;

        if (cancelled) return;
        setSvg(nextSvg);

        // Attach Mermaid interaction handlers (links, callbacks) if provided.
        if (bindFunctions) {
        ...
        if (cancelled) return;
        setSvg(
          `<div style="color: #ef4444; font-size: 0.8em; padding: 1rem; border: 1px dashed #ef4444; border-radius: 0.5rem;">
                        Failed to render diagram. ${
                          message ? `(${message})` : "Check syntax."
                        }
                    </div>`
        );
      }
...
const CustomBlockquote = ({ children }: { children: React.ReactNode }) => {
  ...
  const calloutMarkerStripRegex =
    /^\s*\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]\s*/i;

  const firstParagraphIndex = childrenArray.findIndex(isParagraphElement);
  const firstParagraph =
    firstParagraphIndex >= 0
      ? (childrenArray[firstParagraphIndex] as React.ReactElement)
      : null;
  ...
  if (firstParagraph) {
    ...
    if (match) {
      ...
      let containerClass = "border-blue-500 bg-blue-500/10";
      let titleClass = "text-blue-700 dark:text-blue-200";
      let Icon = Info;
      let title = "Note";
      ...
      switch (type) {
        case "TIP":
          ...
          titleClass = "text-green-700 dark:text-green-200";
          Icon = Lightbulb;
          title = "Tip";
          break;
        case "IMPORTANT":
          containerClass = "border-purple-500 bg-purple-500/10";
          titleClass = "text-purple-700 dark:text-purple-200";
          Icon = AlertTriangle;
          title = "Important";
          break;
        case "WARNING":
          containerClass = "border-yellow-500 bg-yellow-500/10";
      ...
      const stripMarkerOnce = (
        ...
        if (typeof node === "string") {
          if (calloutMarkerStripRegex.test(node)) {
            return {
              nextNode: node.replace(calloutMarkerStripRegex, ""),
              didStrip: true,
            };
          }
      ...
      const filteredChildren = childrenArray
        .map((child, index) => {
          ...
          const nextParagraph = React.cloneElement(
      ...
      return (
        <div
          className={`my-4 border-l-4 rounded-r-md p-4 text-theme-text-main ${containerClass}`}
        >
          <div
            className={`flex items-center gap-2 font-bold mb-2 select-none ${titleClass}`}
          >
            <Icon size={18} />
            <span>{title}</span>
          </div>
          <div className="opacity-90">{filteredChildren}</div>
        </div>
      );
    }
  }
    ...
    className={`
            ...
            ${
              isActive
                ? activeColorClass
                : inactiveColorClass ||
                  "bg-transparent text-theme-text-dim border-transparent hover:text-theme-text-muted hover:bg-theme-activity"
            }
        `}
...
const PreviewArea = React.forwardRef<HTMLDivElement, PreviewAreaProps>(
  ({ onScroll }, ref) => {
    const {
      files,
      activeFileId,
    ...
    const themeCSS = useMemo(
      () => themeService.getThemeCSS(previewTheme),
      [previewTheme]
    );
      ...
      <div className="h-full w-full flex flex-col bg-theme-bg">
        <div className="h-[45px] flex items-center px-3 bg-theme-activity border-b border-theme-border select-none shrink-0 justify-between gap-3">
          ...
          <div className="relative shrink-0">
            ...
            >
              <span className="hidden xl:inline opacity-50 font-normal normal-case tracking-normal">
                Preview:
              </span>
            ...
            {showThemeMenu && (
              <div className="absolute top-full left-0 mt-1 w-36 bg-theme-header border border-theme-border rounded shadow-lg py-1 z-20">
                {(["github", "notion", "minimal"] as PreviewTheme[]).map(
                  (t) => (
                    <button
                      key={t}
                      onClick={() => {
                      ...
                      className={`w-full text-left px-3 py-1.5 text-xs capitalize hover:bg-theme-hover ${
                        previewTheme === t
